"""Evaluationsmetriken für AD (siehe docs/methoden/02_evaluationsprotokoll.md).
...
def best_f1(y_true: np.ndarray, scores: np.ndarray) -> tuple[float, float]:
    """Bester F1 über alle Thresholds + zugehöriger Threshold (threshold-frei wählbar)."""
...
def detection_delay(
    ...
    Läufe ohne Alarm gehen mit der maximal möglichen Verzögerung
    (Samples nach Onset) ein, damit „nie erkannt" bestraft wird.
    """
    df = meta.copy()
    df["score"] = scores
    delays: list[int] = []
