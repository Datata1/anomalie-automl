"""Hyperparameter-Optimierung mit Optuna (AutoML-Strategie 1).

Demonstriert Bayesian Optimization (TPE) auf den AD-Detektoren. Die Zielmetrik wird auf einem
**gelabelten Validierungsset** ausgewertet (Oracle-HPO) — im echten unüberwachten Betrieb
wäre dafür eine interne Metrik nötig (siehe ``automl_ad/automl/selection.py`` und
...
def suggest_params(trial: optuna.Trial, name: str) -> dict:
    """Suchraum je Detektor (siehe Hyperparameter-Tabellen in den AD-Methoden-Docs)."""
    if name == "iforest":
        return {
